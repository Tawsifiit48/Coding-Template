...
void build_y(int node1, int bx, int ex, int node2, int by, int ey) {
    if (by == ey) {
        if (bx == ex)
            ...
            seg[node1][node2] = seg[node1*2][node2] + seg[node1*2+1][node2];
    } else {
        int mid_y = (by + ey) / 2;
        build_y(node1, bx, ex, node2*2, by, mid_y);
        build_y(node1, bx, ex, node2*2+1, mid_y+1, ey);
        seg[node1][node2] = seg[node1][node2*2] + seg[node1][node2*2+1];
    }
}
...
int sum_y(int node1, int node2, int by, int ey, int y1, int y2) {
    if (y1 > y2) {
        return 0;
    }
    if (y1 == by && ey == y2) {
    ...
    int mid_y = (by + ey) / 2;
    return sum_y(node1, node2 * 2, by, mid_y, y1, min(y2, mid_y))
         + sum_y(node1, node2 * 2 + 1, mid_y + 1, ey, max(y1, mid_y + 1), y2);
}

int sum_x(int node, int bx, int ex, int x1, int x2, int y1, int y2) {
    if (x1 > x2) {
        return 0;
    }
    if (x1 == bx && ex == x2) {
        return sum_y(node, 1, 0, m-1, y1, y2);
    }
    int mid = (bx + ex) / 2;
    return sum_x(node * 2, bx, mid, x1, min(x2, mid), y1, y2)
         + sum_x(node * 2 + 1, mid + 1, ex, max(x1, mid + 1), x2, y1, y2);
}
// set n, m to max_n + 1, max_m + 1
